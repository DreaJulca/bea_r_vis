...
p <- ggvis() %>% # our data and mapping layer
  # Our annotation layer goes first!
  layer_paths(data=gdp, x=~Period, y=~Dollars) %>%
    layer_rects(data=annotation, x=~PeriodStart, x2=~PeriodEnd, y=~DollarsMin, y2=~DollarsMax) %>%
...
#	layer_model_predictions(model = "loess", se = TRUE) %>% 
  layer_text(data=annotation, x=~PeriodStart, y=~DollarsMax, #baseline=~label,   #Add our text annotations AFTER all other
            fontSize=3 , angle=45) %>% 
  scale_ordinal("x") %>% 
